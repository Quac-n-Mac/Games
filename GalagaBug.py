...
bullet = Actor("galagabullet")
rock = Actor("galagaplayer")
rock.pos = 600,750
bee = Actor("galagaenemy")

bees=[]
bullets=[]
winner=Actor("galagawon")
...
def on_key_down(key):
    if key == keys.SPACE:
        bullets.append(Actor("galagabullet"))
        bullets[-1].x = rock.x
        bullets[-1].y = rock.y
